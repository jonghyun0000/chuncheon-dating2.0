...
async function switchAdminTab(tab, el) {
  ...
  if (tab === 'users') {
    ...
    if (error) {
      container.innerHTML = `<div class="empty-state">
        <div class="empty-icon">⚠️</div>
        <div class="empty-title">회원 목록 조회 실패</div>
        <div class="empty-desc">${esc(error.message)}</div>
        <div class="empty-desc" style="font-size:11px;color:var(--gray-400);margin-top:8px;">
          Supabase SQL Editor에서 아래 실행:<br>
          DROP POLICY IF EXISTS "u_select_admin" ON users;<br>
          CREATE POLICY "u_select_admin" ON users FOR SELECT<br>
          USING (auth_id = auth.uid() OR<br>
          &nbsp;&nbsp;EXISTS(SELECT 1 FROM users WHERE auth_id=auth.uid() AND role='admin' LIMIT 1));
        </div>
      </div>`;
      return;
    }
