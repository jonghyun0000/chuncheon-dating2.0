...
async function updateBadges() {
  ...
  try {
    ...
    const { data: myTeam } = await _sb
      .from('teams').select('id').eq('leader_id', profile.id)
...
async function submitVerification() {
  ...
  try {
    ...
    if (existingProfile) {
      ...
      // phone_number/custom_badge는 INSERT 제외 → 후속 UPDATE로 저장 (42P17 방지)
      const extraFields = {};
...
async function submitApply() {
  ...
  try {
    ...
    // ★ female_team_id=신청자, male_team_id=피신청자 (성별 무관)
...
async function loadAndRenderRequests(tab) {
  ...
  try {
    ...
    container.innerHTML = data.map(r => {
      const td = r.teams; const tn = td?.title||'-'; const tu = td?.university||'-';
      const lb = SL[r.status]||r.status; const cl = SC[r.status]||'chip-gray';
      const dt = new Date(r.created_at).toLocaleDateString('ko-KR');
      const isM = r.status==='matched'; const isPR = r.status==='pending' && tab==='received';
...
async function _fetchAndShowOpp(oppId) {
  ...
  const pe = document.getElementById('match-contact-phone');
  const ke = document.getElementById('match-contact-kakao');
  if (opp) {
    ...
    if (pe) pe.textContent = phone||'미등록';
    if (ke) ke.textContent = opp.contact_kakao||'미등록';
